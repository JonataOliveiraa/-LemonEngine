...
// Limpamos possíveis variáveis undefined e adicionamos o .js no final automaticamente!
const getRelativePath = (targetModule: string, currentCategory: string, currentFolder: string = '') => {
    // Limpa a string da pasta (garante que não tenha espaços ou barras invertidas)
    const folderClean = currentFolder.replace(/\\/g, '/').trim();
    
    // Conta quantas subpastas existem
    let depth = folderClean ? folderClean.split('/').filter(Boolean).length : 0;
    
    // Se a categoria NÃO for BLANK (ex: ITEM, NPC), ela cria uma pasta extra (Items/, NPCs/)
    if (currentCategory && String(currentCategory).toUpperCase() !== 'BLANK') {
        depth += 1;
    }

    // TUDO que criamos fica dentro da pasta "Content/".
    // Para acessar "TL/" (que está na raiz), subimos +1 nível.
    const totalDepth = depth + 1;
    
    // Retorna as setas de voltar + o Módulo + .js
    return '../'.repeat(totalDepth) + targetModule + '.js';
};

// Garantimos que currentFolder seja no mínimo uma string vazia ''
export const createTlAutocomplete = (currentCategory: string, currentFolder: string = '') => {
        ...
        ...AUTOCOMPLETE_DATA.tlUtils.map(u => ({
          ...
          apply: (view: EditorView, completion: any, from: number, to: number) => {
              ...
              if (!text.includes(impLine)) {
                  ...
                  if (modulePath) {
                      // PASSANDO AS VARIÁVEIS AQUI TAMBÉM:
                      const relPath = getRelativePath(modulePath, currentCategory, currentFolder); 
